...
intentsList = json.loads(train_data_file)

# function for appending stem words
def get_stem_words(wordsList, ignore_wordsList):
    stem_wordsList = []
    for eachWord in wordsList:
        if eachWord not in ignore_wordsList:
            w =stemmerObj.stem(eachWord.lower())
            stem_wordsList.append(w)  
    return stem_wordsList

for eachIntent in intentsList['intents']:
        # Add all words of patterns to list
        for pattern in eachIntent['patterns']:            
            pattern_word = nltk.word_tokenize(pattern)            
            words.extend(pattern_word)                      
            word_tags_list.append((pattern_word, eachIntent['tag']))
        # Add all tags to the classes list
        if eachIntent['tag'] not in classes:
            classes.append(eachIntent['tag'])
...
for word_tags in word_tags_list:
        ...
        for word in pattern_words:
            ...
            word=stemmerObj.stem(word.lower())
